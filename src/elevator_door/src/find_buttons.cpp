...
class ImageConverter
{
    ...
    void imageCb(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        //Occurs until we know we have found all 10 buttons
     if(!found_buttons ){
        //Threshold clone to keep only black buttons
        inRange(src_gray, cv::Scalar(10, 10, 10), cv::Scalar(80, 80, 80), btn_finder); 
            ...
            //cvtColor( btn_finder, btn_finder, CV_BGR2GRAY );
            ...
            for(int i = 0; i < circles.size(); i++){
                ...
                for(int j = 0; j < buttons.size(); j++){
                    ...
                    if(cvRound(circles[i][0]) > cvRound(buttons[j][0]) - 3* radius && 
                            cvRound(circles[i][0]) < cvRound(buttons[j][0] + 3 * radius)) {
                        if(cvRound(circles[i][1]) > cvRound(buttons[j][1]) - 3* radius && 
                                cvRound(circles[i][1]) < cvRound(buttons[j][1] + 3 * radius)){
                            exists = true;
                        }
                    }	
                }
            ...
            if(buttons.size() == 10){
                ...
                for( size_t i = 0; i < buttons.size(); i++ ){
                    ...
                    //give us the location of the actual buttons
                    buttons.at(i)[0] += 2 * buttons.at(i)[2];

                }

                //Identify which button is which
                //sorting algorithm for x's, then sort in groups of 3 from there to find each button and their corresponding floor number
                ...
                //Removes the buttons at the bottom which are not the floors
	...
	/*
		for(int k = 0; k < buttons.size(); k++){
			int x = cvRound(buttons.at(k)[0]);
                  	  int y = cvRound(buttons.at(k)[1]);
                   	 int radius = cvRound(buttons.at(k)[2]);
			D[k](A, Rect(x - radius, y - radius, 2* radius, 2 * radius));
		}
*/
